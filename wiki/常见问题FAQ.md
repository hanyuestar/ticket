...
**Q11：查询报「查询余票请求失败 状态码 302」？**
A：v1.0.2 已内置自动处理——程序会用无头浏览器预热会话（导入 12306 反爬 Cookie），被拦截时自动自愈（60 秒冷却）。若仍持续 302：① 升级到 v1.0.2+；② 确认运行环境能访问 `kyfw.12306.cn`；③ 云服务器/IDC IP 段更易被风控，建议家宽环境运行；④ 极端情况下可在浏览器登录 12306 后按 F12 提取 `RAIL_DEVICEID`/`RAIL_EXPIRATION` 填入 `env.py` 并设 `CACHE_RAIL_ID_ENABLED=1`。

**Q12：启动时出现「正在通过无头浏览器预热会话」正常吗？**
A：正常。这是 v1.0.2 的会话预热（约 10~30 秒），预热完成后查询走原生 requests，速度不受影响；只有预热/自愈时会短暂启动浏览器。
